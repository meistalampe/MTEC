...
image_emotion_evaluation_neg = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128, interpolate=True, depth=0.0)
rating_emotion_evaluation_neg = visual.RatingScale(win=win, name='rating_emotion_evaluation_neg', marker='triangle', size=1.0, pos=[0.0, -0.4], low=1, high=4, labels=['negative', 'positive'], scale='', markerStart='3')
...
for thisTick in tick:
    ...
    tick.addData('sound_1.started', sound_1.tStartRefresh)
    tick.addData('sound_1.stopped', sound_1.tStopRefresh)
...
# abbreviate parameter names if possible (e.g. rgb = thisEmotion_evaluation_po.rgb)
if thisEmotion_evaluation_po != None:
    for paramName in thisEmotion_evaluation_po:
        exec('{} = thisEmotion_evaluation_po[paramName]'.format(paramName))

for thisEmotion_evaluation_po in emotion_evaluation_pos:
    currentLoop = emotion_evaluation_pos
    # abbreviate parameter names if possible (e.g. rgb = thisEmotion_evaluation_po.rgb)
    if thisEmotion_evaluation_po != None:
        for paramName in thisEmotion_evaluation_po:
            exec('{} = thisEmotion_evaluation_po[paramName]'.format(paramName))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    image_emotion_evaluation_pos.setSize((new_width*0.3, new_height*0.3))
    ...
    for thisComponent in Emotion_Evaluation_PosComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    emotion_evaluation_pos.addData('image_emotion_evaluation_pos.started', image_emotion_evaluation_pos.tStartRefresh)
    emotion_evaluation_pos.addData('image_emotion_evaluation_pos.stopped', image_emotion_evaluation_pos.tStopRefresh)
    # store data for emotion_evaluation_pos (TrialHandler)
    emotion_evaluation_pos.addData('rating_emotion_evaluation_pos.response', rating_emotion_evaluation_pos.getRating())
    emotion_evaluation_pos.addData('rating_emotion_evaluation_pos.rt', rating_emotion_evaluation_pos.getRT())
    emotion_evaluation_pos.addData('rating_emotion_evaluation_pos.started', rating_emotion_evaluation_pos.tStart)
    emotion_evaluation_pos.addData('rating_emotion_evaluation_pos.stopped', rating_emotion_evaluation_pos.tStop)
...
# completed 1 repeats of 'emotion_evaluation_pos'

# get names of stimulus parameters
if emotion_evaluation_pos.trialList in ([], [None], None):
    params = []
else:
    params = emotion_evaluation_pos.trialList[0].keys()
# save data for this loop
emotion_evaluation_pos.saveAsExcel(filename + '.xlsx', sheetName='emotion_evaluation_pos',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
emotion_evaluation_pos.saveAsText(filename + 'emotion_evaluation_pos.csv', delim=',',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
...
# abbreviate parameter names if possible (e.g. rgb = thisEmotion_evaluation_neg.rgb)
if thisEmotion_evaluation_neg != None:
    for paramName in thisEmotion_evaluation_neg:
        exec('{} = thisEmotion_evaluation_neg[paramName]'.format(paramName))

for thisEmotion_evaluation_neg in emotion_evaluation_neg:
    currentLoop = emotion_evaluation_neg
    # abbreviate parameter names if possible (e.g. rgb = thisEmotion_evaluation_neg.rgb)
    if thisEmotion_evaluation_neg != None:
        for paramName in thisEmotion_evaluation_neg:
            exec('{} = thisEmotion_evaluation_neg[paramName]'.format(paramName))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    image_emotion_evaluation_neg.setSize((new_width*0.3, new_height*0.3))
    ...
    for thisComponent in Emotion_Evaluation_NegComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    emotion_evaluation_neg.addData('image_emotion_evaluation_neg.started', image_emotion_evaluation_neg.tStartRefresh)
    emotion_evaluation_neg.addData('image_emotion_evaluation_neg.stopped', image_emotion_evaluation_neg.tStopRefresh)
    # store data for emotion_evaluation_neg (TrialHandler)
    emotion_evaluation_neg.addData('rating_emotion_evaluation_neg.response', rating_emotion_evaluation_neg.getRating())
    emotion_evaluation_neg.addData('rating_emotion_evaluation_neg.rt', rating_emotion_evaluation_neg.getRT())
    emotion_evaluation_neg.addData('rating_emotion_evaluation_neg.started', rating_emotion_evaluation_neg.tStart)
    emotion_evaluation_neg.addData('rating_emotion_evaluation_neg.stopped', rating_emotion_evaluation_neg.tStop)
...
# completed 1 repeats of 'emotion_evaluation_neg'

# get names of stimulus parameters
if emotion_evaluation_neg.trialList in ([], [None], None):
    params = []
else:
    params = emotion_evaluation_neg.trialList[0].keys()
# save data for this loop
emotion_evaluation_neg.saveAsExcel(filename + '.xlsx', sheetName='emotion_evaluation_neg',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
emotion_evaluation_neg.saveAsText(filename + 'emotion_evaluation_neg.csv', delim=',',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
